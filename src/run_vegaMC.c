...
/* Dataset properties */
  ...
  int run_vegaMC(double *M, int *row, int *col,
                 char **out, double *b, int *mrbs, double *losst,
  ...
  void load_data()
  {
    ...
    */
    ...
    for (j = 0; j < nrow; ++j){
      ...
      for (i = 0; i < ncol; ++i) {
        if (i == 0) {
                chr = (int) matrix[i*nrow + j]; 
          if (prev_chr != chr) {
            num_chromosomes++;
            prev_chr = chr;
          }
        }else if (i == 1) {
          position = (int) matrix[i*nrow + j];  
        }else {
          data_tmp[i - 2] = (float) matrix[i*nrow + j];
        }
      }
